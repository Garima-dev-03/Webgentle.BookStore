...
namespace Webgentle.BookStore.Repository
{
    public class MovieRepository 
    {
        ...
        public void AddToPlaylist(MovieModel movieModel)
        {
             _databaseContext.MovieDetails.Add(movieModel);
            
        }
        public List<MovieModel> GetAllBooks()
        {
            return _databaseContext.MovieDetails.ToList();
        }
        public MovieModel GetMovieById(int id)
        {
            return _databaseContext.MovieDetails.Where(x => x.Id == id).FirstOrDefault();
        }
        public List<MovieModel> SearchBooks(string title, string directorName)
        {
            ...
            return _databaseContext.MovieDetails.Where(x => x.Title.Contains(title) && x.Director.Contains(directorName)).ToList();
        }
        ...
        //        new MovieModel(){Id = 3,Title ="Vampire Diaries",Director="Harry",Description="Wonder Woman is a superheroine appearing in American comic books published by DC Comics. ... In her homeland, the island nation of Themyscira, her official title is Princess Diana of Themyscira. When blending into the society outside of her homeland, she sometimes adopts her civilian identity Diana Prince."},
